...
export const Card = (props) => {
  ...
  return (
    <>
      {modalIsOpen && <PopUpContacts handleShowingModal={() => window.history.back()} />}
      {props.isEdit
...
const renderCardContent = ({ name, gender, age, photos, showModal }) => {
  return <li className="animal-details-card w-full flex flex-col">
...
const renderCardContentForEdit = ({ name, type, gender, age, updateName, updateType, updateGender, updateAge, nameRef, typeRef, genderRef, isEdit }) => {
  return <li className="animal-details-card w-full flex flex-col">
    <div className="w-11/12 p-4 bg-white text-center text-gray-700 uppercase shadow-xl rounded-b-xl self-center">
      <p className="font-bold text-lg outline-none px-2 bg-orange-200"
        contentEditable
      ...
      <div className="flex justify-around items-center my-4">
        <select
          ...
          {type === 'кот'
            ? <>
              <option value='кот' selected>Кот</option>
              <option value='пес'>Пес</option>
            </>
            : <>
              <option value='кот'>Кот</option>
              <option value='пес' selected>Пес</option>
            </>
          }
        </select>
        <select
        ...
        >
          {gender === 'М'
            ? <>
              <option value='М' selected>Мальчик</option>
              <option value='Ж'>Девочка</option>
            </>
            : <>
              <option value='М'>Мальчик</option>
              <option value='Ж' selected>Девочка</option>
            </>
          }
        </select>
      </div>
